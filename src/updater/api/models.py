...
class DownloadRequest(BaseModel):
    ...
    package_url: str = Field(
        ..., pattern=r"^https://.+", description="HTTPS URL to download package"
    )
    package_name: str = Field(..., description="Filename for local storage")
    package_size: int = Field(..., gt=0, description="Total size in bytes")
    package_md5: str = Field(
        ..., pattern=r"^[a-f0-9]{32}$", description="Expected MD5 hash (32-char hex)"
    )
